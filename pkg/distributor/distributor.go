...
var (
	...
	errMaxInflightBytes = errors.New("the server has too much data at this time to process your request")
...
func (cfg *Config) RegisterFlags(fs *flag.FlagSet) {
	...
	fs.IntVar(&cfg.MaxInflightBytes, "distributor.max-inflight-bytes", 2<<29, "The maximum number of inflight bytes at a time.")
...
func (cfg *Config) Validate() error {
	...
	if cfg.MaxInflightBytes < 0 {
		return errors.New("max inflight bytes cannot be negative")
	}
	return nil
}
...
func New(
	...
	if cfg.CircuitBreaker.Enabled {
		circuitBreaker := newTrialCircuitBreaker(
			...
			func(err error) bool {
				return errors.Is(err, kgo.ErrMaxBuffered) || errors.Is(err, errMaxInflightBytes)
			},
		)
...
func (d *Distributor) PushWithResolver(ctx context.Context, req *logproto.PushRequest, streamResolver *requestScopedStreamResolver, format string) (*logproto.PushResponse, error) {
	...
	if maxInflightBytes > 0 && newInflightBytes > maxInflightBytes {
		return nil, errMaxInflightBytes
	}
